  ...
  ArrangementEntry(entry, i) {
    ...
    switch (entry.component) {
      case "event":
        return (
          <Card
            name={ConvertName(entry.options.title)}
            artist={entry.options.title}
            date={entry.options.date}
            description={entry.options.description}
            icon={entry.options.featureImage}
          />
        );
      default:
        // eslint-disable-next-line no-console
  ...
  render() {
    ...
    return (
      <AsyncComponent
        ...
        Load={async () => await this.props.siteStore.LoadSite("iq__2iXyBxthQCSKh7Tr5CmdsSCkjV4r", "")}
